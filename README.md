...
You can train and test the KenLM model in two different ways:
- Method 1: Using the provided Makefile
- Method 2: Manually running all stages from scratch

## Method 1: Using the provided Makefile
- Step 1: For extracting paths from PD graphs, run the following command:
  - `make paths`
- Step 2: For creating a training corpus, run the following command:
  - `make corpus`
- Step 3: To install necessary dependencies and for training the KenLM model, run the following command:
  - `make kenlm_model`
- Step 4: To create the vocabulary, run the following command:
  - `make vocabulary`
- Step 5: To evaluate the KenLM model, run the following command:
  - `make mrr`
- Optional: To run tests, run the following command:
  - `make test`         

The Makefile first runs a Python script to extract paths from PD graphs based on training hashes and a database. It then merges these paths into a training corpus. Afterward, it installs the required dependencies, builds KenLM, and trains a language model using the corpus. Following this, a vocabulary is generated from the trained model. The corpus, trained model, and vocabulary file are stored in the `trained_models` directory. Finally, the model is evaluated using MRR (Mean Reciprocal Rank) and the results are stored in the `output` directory and `mrr.txt` file. You can modify the training and test hash file locations specified in the Makefile to any of the 5 training-test split locations available in the repository.

## Method 2: Step 1: Extracting paths from our PD graphs

Since KenLM is a sequential model and the structure of PD files resembles a directed graph, we extract paths from the PD graphs by analyzing their connections (edges). This can be done using the following commands from the root directory of this project:
- `mkdir paths_all`
...
## Method 2: Step 2: Create a training corpus
After extracting the paths from the PD graphs, we create our training corpus by merging all the paths generated from the graphs into a single file. This can be done by running the following commands from the root directory of this project:
...
## Method 2: Step 3: Training the KenLM model and creating the .arpa file
...
This command will generate a KenLM 3-gram model trained on your corpus and save it in the `trained_models` directory as `kenlm_3.arpa`. We have also included a sample `.arpa` file in the `sample_trained_models` directory, trained on our dataset, named `kenlm_3_paths_all_not_padded.arpa`. After this step, change your directory back to your root directory by running `cd ../../`.
...
## Method 2: Step 4: Create vocabulary

To evaluate the performance of our KenLM model, we first need to create a vocabulary file that represents all the unique tokens in our corpus. You can generate the vocabulary file by running the following script from the root directory of our project:
...
## Method 2: Step 5: Evaluate the KenLM model

To evaluate the KenLM model's performance on the test hashes of the PD graphs in your test set, run the following commands from the root directory of our project:
- `mkdir output`
